...
class VPNApiClient {
    ...
    func getStatus() async throws -> [String: Any] {
        guard var urlComponents = URLComponents(string: "\(baseURL)/vpn/status") else {
            throw VPNError.invalidURL
        }
        
        // Add required action query parameter (though its purpose isn't specified in the Swagger)
        urlComponents.queryItems = [URLQueryItem(name: "action", value: "status")]
